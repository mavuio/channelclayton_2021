defmodule MyAppBe.TwMaterialInputTheme do
  ...
  def default_options(form, field, opts) do
    ...
    if field == :season_id do
      {form, field}
      |> IO.inspect(label: "mwuits-debug 2021-06-25_12:21 {form, field} select defaults")
    end

    if opts[:using] == :select do
      defopts
      |> Keyword.put(:"data-value", Phoenix.HTML.Form.input_value(form, field) |> to_string())
